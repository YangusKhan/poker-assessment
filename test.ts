...
describe('Tests', () => {
  ...
  it('Should handle tie-breakers', () => {
    const hand1 = 'AS,10C,8H,2D,5H';
    const hand2 = 'AD,10H,8D,2C,4S';
    const result = new Hand(hand1).winsOver(new Hand(hand2));
    assert(result, `${hand1} should win!`);
  });

  it("Should determine straight hands", () => {
    const hand = "3C,4S,5D,6H,7C";
    const result = new Hand(hand).isStraight();
    assert(result, `${hand} should be a straight!`);
  })

  it("Should determine flush hands", () => {
    const hand = "3C,JC,5C,KC,2C";
    const result = new Hand(hand).isFlush();
    assert(result, `${hand} should be a flush!`);
  })

  it("Should determine straight-flush hands", () => {
    const cards = "3C,4C,5C,6C,7C";
    const hand = new Hand(cards);
    const result = hand.isStraight() && hand.isFlush();
    assert(result, `${cards} should be a straight flush!`);
  })
});
